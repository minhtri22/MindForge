"""OIR-PPV H3R v1.1 preflight and one-shot decisive execution.
...
PROTOCOL_MD = ROOT / "protocols/h3r/H3R_PROTOCOL_v1.1.md"
PROTOCOL_JSON = ROOT / "protocols/h3r/h3r_protocol_v1.1.json"
TEST_MANIFEST = ROOT / "protocols/h3r/h3r_test_manifest_v1.1.json"
FREEZE_MANIFEST = ROOT / "governance/H3R_v1.1_freeze_manifest.json"
V1_0_TEST_MANIFEST = ROOT / "protocols/h3r/h3r_test_manifest_v1.0.json"
LEARNER_MANIFEST = ROOT / "experiments/OIR_PPV/Learner_Benchmark/EXP-LRN-001/matrix_manifest.json"
PLAN_PATH = ROOT / "PLAN.md"
ACCESS_LOG = ROOT / "protocols/h3r/h3r_test_access_log_v1.1.md"
DEFAULT_EXECUTION_ROOT = ROOT / "experiments/OIR_PPV/H3R/EXP-H3R-002"
PROTOCOL_VERSION = "v1.1"
H3R_SEEDS = (223691, 965182, 537173, 538839, 124586)
SMOKE_SEED = 424242
...
def _stable_scalar_bytes(value: Any) -> bytes:
    """Canonical scalar encoding for mixed object tables.

    Numpy object-array ``tobytes()`` serializes object pointers, which is not a
    stable scientific identity across processes.  H3R v1.1 hashes the clean
    observation values themselves while preserving type distinctions.
    """
    ...
    raise H3RProtocolError(f"Unsupported observation scalar type for stable hashing: {type(value)!r}")


def _stable_observation_bytes(value: np.ndarray) -> bytes:
    array = np.asarray(value)
    header = json.dumps(
        {"encoding": "H3R_OBSERVATION_VALUES_V1", "shape": list(array.shape)},
        separators=(",", ":"),
        sort_keys=True,
    ).encode("utf-8")
    chunks = [len(header).to_bytes(8, "big"), header]
    for scalar in array.reshape(-1):
        encoded = _stable_scalar_bytes(scalar)
        chunks.append(len(encoded).to_bytes(8, "big"))
        chunks.append(encoded)
    return b"".join(chunks)


def _sha256_observation(value: np.ndarray) -> str:
    return _sha256_bytes(_stable_observation_bytes(value))
...
def validate_static_contract(*, require_pristine_access_log: bool = True) -> dict[str, Any]:
    ...
    check("h3r_seed_set", frozen.test_manifest["h3r_seeds"] == list(H3R_SEEDS))
    check("cell_count", frozen.test_manifest["cell_count"] == 20)
    check(
        "total_test_rows",
        frozen.test_manifest["total_test_rows"]
        == sum(int(cell["test_count"]) for cell in frozen.test_manifest["cells"]),
    )
    check(
        "observation_hash_encoding",
        frozen.test_manifest.get("observation_hash_encoding") == "H3R_OBSERVATION_VALUES_V1",
    )
...
def seal_test_lock(output_path: Path = TEST_MANIFEST, access_log_path: Path = ACCESS_LOG) -> dict[str, Any]:
    """Prospectively seal v1.1 test identity without model/metric execution."""
    if output_path.exists() or access_log_path.exists():
        raise H3RProtocolError("Refusing to overwrite an existing H3R v1.1 test lock/access log")
    anchor = _load_json(V1_0_TEST_MANIFEST)
    excluded = set(int(seed) for seed in anchor["historical_test_seeds_excluded"])
    excluded.update(int(seed) for seed in anchor["h3r_seeds"])
    if excluded.intersection(H3R_SEEDS):
        raise H3RProtocolError("Fresh H3R v1.1 seed set overlaps historical/v1.0 test seeds")

    source_hashes = dict(anchor["source_hashes"])
    for relative, expected in source_hashes.items():
        actual = _sha256_path(ROOT / relative)
        if actual != expected:
            raise H3RProtocolError(f"Generator/source lineage drift before v1.1 seal: {relative}")
    source_hashes["pipeline/run_h3r_v11.py"] = _sha256_path(Path(__file__))
    source_hashes["tests/test_h3r_v11_execution.py"] = _sha256_path(
        ROOT / "tests/test_h3r_v11_execution.py"
    )

    lineage = FrozenInputs(
        protocol={},
        test_manifest=anchor,
        freeze_manifest={},
        learner_manifest={},
    )
    cells: list[dict[str, Any]] = []
    for environment_id in anchor["environments"]:
        for seed in H3R_SEEDS:
            data = generate_frozen_cell(environment_id, int(seed), lineage)
            test_idx = np.asarray(data.splits["test"])
            clean_obs = np.asarray(data.observations[test_idx])
            labels = np.asarray(data.labels[test_idx])
            cells.append(
                {
                    ...
                    "test_observation_sha256": _sha256_observation(clean_obs),
    ...
    timestamp = _utc_now()
    manifest = {
        "manifest_id": "H3R-TEST-LOCK-v1.1",
        "protocol_id": "OIR-PPV-H3R",
        "protocol_version": PROTOCOL_VERSION,
        "status": "FROZEN_TEST_IDENTITY_NOT_EXECUTED",
        "generation_timestamp_utc": timestamp,
        "source_anchor_head": _git("rev-parse", "HEAD"),
        "historical_test_seeds_excluded": anchor["historical_test_seeds_excluded"],
        "v1_0_test_seeds_excluded": anchor["h3r_seeds"],
        "h3r_seeds": list(H3R_SEEDS),
        "environments": anchor["environments"],
        "environment_generation_overrides": anchor["environment_generation_overrides"],
        "source_hashes": source_hashes,
        "observation_hash_encoding": "H3R_OBSERVATION_VALUES_V1",
        "noise_contract": {
            "norm": "TRAIN_SCALE_NORMALIZED_L_INF",
            "delta": NOISE_DELTA,
            "distribution": "INDEPENDENT_UNIFORM_PER_NUMERIC_CHANNEL",
            "variables_affected": "all numeric observation channels only; train-derived mask",
            "replicates_per_test_row": NOISE_REPLICATES,
            "seed_derivation": (
                "uint32(SHA256('OIR-PPV-H3R|v1.1|<environment>|<generator_seed>|"
                "<test_row_identity>|<replicate_index>'))"
            ),
            "pairing": "same clean test row and same noisy observation across L0-L4",
        },
        "cells": cells,
        "cell_count": len(cells),
        "total_test_rows": int(sum(cell["test_count"] for cell in cells)),
        "scientific_test_access_count_at_freeze": 0,
        "test_unlock_timestamp": None,
    }
    ...
    access_log_path.write_text(
        ...
        "| --- | --- | --- | --- | --- |\n"
        f"| {timestamp} | deterministic generation/sealing pass used only to record test split counts and stable SHA256 identities for test indices, raw clean observation values, and labels; no candidate was run and no metric/outcome summary was inspected | no | no | no |\n\n"
    ...
    return {
        "status": "SEALED_WITHOUT_SCIENTIFIC_METRIC_ACCESS",
        "cell_count": len(cells),
        "total_test_rows": manifest["total_test_rows"],
        "test_manifest_sha256": _sha256_path(output_path),
        "scientific_test_access_count": 0,
    }
    ...
    for environment_id in frozen.test_manifest["environments"]:
        for seed in frozen.test_manifest["h3r_seeds"]:
            ...
            actual = {
                ...
                "test_observation_sha256": _sha256_observation(obs),
                "test_label_sha256": _sha256_array(labels),
            }
            passed = (
...
def h3r_noise_seed(environment_id: str, generator_seed: int, row_identity: int, replicate_index: int) -> int:
    material = (
        f"OIR-PPV-H3R|{PROTOCOL_VERSION}|{environment_id}|{int(generator_seed)}|"
...
def _as_2d_observation(values: np.ndarray) -> np.ndarray:
    values = np.asarray(values)
    return values.reshape(-1, 1) if values.ndim == 1 else values


def _numeric_channel_mask_from_train(train_obs: np.ndarray) -> np.ndarray:
    """Classify perturbable channels from train observations only."""
    table = _as_2d_observation(train_obs)
    mask = np.zeros(table.shape[1], dtype=bool)
    for index in range(table.shape[1]):
        try:
            numeric = table[:, index].astype(float)
        except (TypeError, ValueError):
            continue
        if np.isfinite(numeric).all():
            mask[index] = True
    if not mask.any():
        raise H3RProtocolError("No numeric observation channel available for H3R noise")
    return mask


def _train_scale(train_obs: np.ndarray, numeric_mask: np.ndarray) -> np.ndarray:
    table = _as_2d_observation(train_obs)
    mask = np.asarray(numeric_mask, dtype=bool)
    if mask.shape != (table.shape[1],):
        raise H3RProtocolError("Numeric-channel mask does not match observation width")
    numeric = np.asarray(table[:, mask], dtype=float)
...
def make_noisy_replicates(
    ...
    clean = _as_2d_observation(clean_obs)
    mask = np.asarray(numeric_mask, dtype=bool)
    if mask.shape != (clean.shape[1],):
        raise H3RProtocolError("Numeric-channel mask does not match clean observation width")
    scale = np.asarray(train_scale, dtype=float)
    numeric_clean = np.asarray(clean[:, mask], dtype=float)
    if scale.shape != (numeric_clean.shape[1],):
        raise H3RProtocolError("Train scale does not match numeric-channel count")
    categorical_mask = ~mask
    categorical_hash = _sha256_observation(clean[:, categorical_mask])
    ...
    for replicate_index in range(NOISE_REPLICATES):
        ...
        for row_offset, row_identity in enumerate(row_identities):
            ...
            eta = NOISE_DELTA * scale * rng.uniform(-1.0, 1.0, size=int(mask.sum()))
            noisy[row_offset, mask] = numeric_clean[row_offset] + eta
        noisy_numeric = np.asarray(noisy[:, mask], dtype=float)
        normalized = np.abs(noisy_numeric - numeric_clean) / scale
        ...
        if _sha256_observation(noisy[:, categorical_mask]) != categorical_hash:
            raise H3RProtocolError("Categorical observation channel changed under numeric-only noise")
        replicate_hashes.append(_sha256_observation(noisy))
    ...
    return noisy_replicates, {
        ...
        "numeric_channel_indices": np.flatnonzero(mask).astype(int).tolist(),
        "categorical_channel_indices": np.flatnonzero(categorical_mask).astype(int).tolist(),
        "numeric_mask_source": "train_observations_only",
...
def _runtime_manifest() -> dict[str, Any]:
    ...
    return {
        ...
        "source_hashes": {
            "pipeline/run_h3r_v11.py": _sha256_path(Path(__file__)),
...
def _append_scientific_access_event(authorization: dict[str, Any]) -> str:
    ...
    if "H3R_V1_1_DECISIVE_ACCESS_001" in current:
        raise H3RProtocolError("Decisive scientific test access is already recorded; rerun refused")
    timestamp = _utc_now()
    line = (
        f"\n| {timestamp} | H3R_V1_1_DECISIVE_ACCESS_001; owner-authorized one-shot H3R v1.1 "
    ...
    try:
        for environment_id in frozen.test_manifest["environments"]:
            for seed in frozen.test_manifest["h3r_seeds"]:
                ...
                if (
                    ...
                    or _sha256_observation(clean_test_obs) != expected["test_observation_sha256"]
                ...
                numeric_mask = _numeric_channel_mask_from_train(train_obs)
                scale = _train_scale(train_obs, numeric_mask)
                noisy_replicates, validator = make_noisy_replicates(
                    clean_test_obs, scale, numeric_mask, environment_id, seed, test_idx
                )
        ...
        summary = {
            ...
            "matrix": {
                "valid_cells": 20,
                "expected_cells": 20,
                "total_test_rows": int(frozen.test_manifest["total_test_rows"]),
            },
...
def main() -> int:
    parser = argparse.ArgumentParser(description="OIR-PPV H3R v1.1 runner")
    parser.add_argument("--mode", choices=("seal-test-lock", "preflight", "execute"), required=True)
    parser.add_argument("--output", type=Path)
    parser.add_argument("--authorization", type=Path)
    args = parser.parse_args()

    if args.mode == "seal-test-lock":
        output = args.output or TEST_MANIFEST
        result = seal_test_lock(output, ACCESS_LOG)

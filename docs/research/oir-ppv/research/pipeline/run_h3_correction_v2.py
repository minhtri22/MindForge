"""Versioned H3 provenance and causal-isolation correction replay (v2)."""
...
EXP_V2 = EXP_V1 / "correction_v2"
ART_V2 = ART_V1 / "correction_v2"
PRESERVATION = EXP_V2 / "v1_preservation_manifest.json"
SOURCE_MANIFEST = EXP_V2 / "source_manifest_v2.json"
RUNTIME_MANIFEST = EXP_V2 / "runtime_environment_v2.json"
EXEC_MANIFEST = EXP_V2 / "h3_execution_manifest_v2.json"
EXEC_MANIFEST_HASH = EXP_V2 / "h3_execution_manifest_v2.sha256"
SMOKE = EXP_V2 / "h3_smoke_evidence_v2.json"
SUMMARY = EXP_V2 / "h3_summary_v2.json"
REPORT = EXP_V2 / "H3_CLOSURE_REPORT_v2.md"
RECONCILIATION = EXP_V2 / "artifact_reconciliation_v2.json"
MATRIX_EXECUTION = ART_V2 / "matrix_execution_v2.json"
EXECUTION_LOG = ART_V2 / "execution_log_v2.jsonl"
...
COMMANDS = {
    "freeze": "python pipeline/run_h3_correction_v2.py freeze",
    "smoke": "python pipeline/run_h3_correction_v2.py smoke",
    "matrix": "python pipeline/run_h3_correction_v2.py run",
    "aggregate": "python pipeline/run_h3_correction_v2.py aggregate",
    "reconcile": "python pipeline/run_h3_correction_v2.py reconcile",
    "verify_v1_after": "python pipeline/run_h3_correction_v2.py verify-v1-after",
    "compile": "python -m py_compile pipeline/run_h3_closure.py pipeline/run_h3_correction_v2.py",
    "focused_tests": "python -m pytest tests/test_h3_correction_v2.py -q -p no:cacheprovider --basetemp artifacts/h3_v2_pytest_focused_20260909",
    "regression_tests": "python -m pytest tests/test_h3_closure.py tests/test_h2_closure.py tests/test_learner_contract.py tests/test_real_learners.py tests/test_h3_correction_v2.py -q -p no:cacheprovider --basetemp artifacts/h3_v2_pytest_regression_20260909",
}
...
def _source_paths() -> list[Path]:
    paths = [
        ...
        ROOT / "pipeline/run_h3_correction_v2.py",
        ROOT / "tests/test_h3_closure.py",
        ROOT / "tests/test_h3_correction_v2.py",
...
def _source_manifest_payload() -> dict:
    ...
    return {"version": 2, "hash_algorithm": "SHA256", "files": files, "learners": learner_rows}
...
def freeze() -> dict:
    if any(p.exists() for p in (SOURCE_MANIFEST, RUNTIME_MANIFEST, EXEC_MANIFEST, EXEC_MANIFEST_HASH)):
        raise FileExistsError("Refusing to overwrite v2 freeze files")
    ...
    manifest = {
        ...
        "correction_namespace": "correction_v2",
        "status": "frozen_before_correction_replay",
        ...
        "scientific_reconciliation_rule": "project every v2 object onto v1 scientific keys, excluding provenance; exact JSON equality; additive v2 provenance/causal-isolation fields ignored",
...
def _paired_interventions_v2(prepared: dict, env_id: str, seed: int) -> tuple[str | None, list[Any], list[dict]]:
...
def _run_cell_v2(learner: str, env_id: str, seed: int, output_root: Path) -> dict:
    ...
    result = {
        ...
        "provenance": {
            ...
            "h3_execution_manifest_v2_sha256": _sha(EXEC_MANIFEST), "source_manifest_v2_sha256": _sha(SOURCE_MANIFEST),
            "runtime_environment_v2_sha256": _sha(RUNTIME_MANIFEST), "canonical_hash_algorithm": CANONICAL_HASH_ALGORITHM,
        },
    }
...
def smoke() -> dict:
    _verify_frozen_identity()
    if SMOKE.exists():
        raise FileExistsError("Refusing to overwrite v2 smoke")
    smoke_root = ART_V2 / "smoke"
    baseline = _run_cell_v2("L0", "ENV-4", 42, smoke_root)
    candidate = _run_cell_v2("L1", "ENV-4", 42, smoke_root)
    ...
    for learner in LEARNERS:
        for env_id in ENVS:
            for seed in SEEDS:
                cell_id = f"{learner}-{env_id}-S{seed}"
                try:
                    result = _run_cell_v2(learner, env_id, seed, ART_V2 / "full")
                    statuses[result["status"]] += 1
                except Exception as exc:
                    statuses["failed"] += 1
                    failure = {"cell_id": cell_id, "status": "failed", "error": repr(exc)}
                    failures.append(failure)
                    fp = ART_V2 / "full" / cell_id / "failure_v2.json"
                    if not fp.exists():
                    ...
                    if isinstance(exc, CorrectionStop) and "CAUSAL_ISOLATION_FAILURE" in str(exc):
                        _write_json(MATRIX_EXECUTION, {"experiment_id":"EXP-H3-001","version":2,"counts":statuses,"failures":failures,"status":"CAUSAL_ISOLATION_FAILURE"}, refuse_overwrite=True)
                        raise
    result = {"experiment_id": "EXP-H3-001", "version": 2, "counts": statuses, "failures": failures, "attempted_cells": sum(statuses.values()), "one_attempt_no_retry": True}
    _write_json(MATRIX_EXECUTION, result, refuse_overwrite=True)
    _log("matrix_complete", command=COMMANDS["matrix"], **result)
    return result


def _load_v2_cell(learner: str, env_id: str, seed: int) -> dict | None:
    p = ART_V2 / "full" / f"{learner}-{env_id}-S{seed}" / "paired_evidence_v2.json"
...
def _compute_summary() -> dict:
    ...
    for learner in LEARNERS:
        for env_id in ENVS:
            cells = [_load_v2_cell(learner, env_id, seed) for seed in SEEDS]
    ...
    for learner in CANDIDATES:
        ...
        for env_id in ELIGIBLE_ENVS:
            for seed in SEEDS:
                cand=_load_v2_cell(learner,env_id,seed); base=_load_v2_cell(BASELINE,env_id,seed); pair_id=f"{learner}-vs-{BASELINE}-{env_id}-S{seed}"
    ...
    for l in LEARNERS:
        for e in ENVS:
            for s in SEEDS:
                c=_load_v2_cell(l,e,s); counts[c["status"] if c else "failed"]+=1
...
def reconcile() -> dict:
    ...
    for l in LEARNERS:
        for e in ENVS:
            for s in SEEDS:
                v1p=ART_V1/"full"/f"{l}-{e}-S{s}"/"paired_evidence.json"; v2p=ART_V2/"full"/f"{l}-{e}-S{s}"/"paired_evidence_v2.json"
                ...
                records.append({"cell_id":f"{l}-{e}-S{s}","v1_path":_rel(v1p),"v1_sha256":_sha(v1p),"v2_path":_rel(v2p),"v2_sha256":_sha(v2p),"compared_field_encoding_bytes":count,"mismatch_count":len(diffs),"mismatches":diffs})
...
def _write_report(recon: dict) -> None:
    ...
    vals={}
    for e in ENVS:
        c=_load_v2_cell("L0",e,42); vals[e]=c["nuisance_values"] if c else []
    lines=["# H3 Closure Report v2 — EXP-H3-001 correction","",f"Status: `READY_FOR_QA_REVIEW`  ","Authority: `DEVELOPER_CANDIDATE_ONLY`",f"Global developer candidate: `{s['global_developer_candidate_h3']}`","","## Exact intervention coverage","", "| Environment | Selected target | Three frozen values | Eligibility |","| --- | --- | --- | --- |"]
    for e in ENVS: lines.append(f"| {e} | `{TARGETS[e]}` | `{vals[e]}` | {'inapplicable: do(N) changes Y/task labels' if e=='ENV-3' else 'task-preserving eligible'} |")
    ...
    lines += ["",f"Matrix: `{s['matrix_counts']}`. v1/v2 reconciliation: `{recon['status']}`, mismatches=`{recon['mismatch_count']}`.","", "## Per-seed evidence","", "| Learner | Env | Seed | Status | Target | |Delta_N| | Leak_N | Utility |","| --- | --- | ---: | --- | --- | ---: | ---: | ---: |"]

# H3 Closure Report v3 — EXP-H3-001 correction retry
...
The selected nuisance target is fixed per environment. ENV-1 and ENV-4 numeric intervention triplets are seed-dependent under the frozen target-selection algorithm, so the table enumerates the exact three values for every frozen environment/seed combination.

| Environment | Seed | Selected target | Exact three frozen values | Eligibility |
| --- | ---: | --- | --- | --- |
| ENV-1 | 42 | `background_noise` | `[-0.13230397117893647, 0.00442170029001831, 0.12414274167932847]` | task-preserving eligible |
| ENV-1 | 123 | `background_noise` | `[-0.1301844227336907, 0.027973921341989574, 0.1715021970486985]` | task-preserving eligible |
| ENV-1 | 456 | `background_noise` | `[-0.1325623193259564, 0.003018862493272322, 0.13240441948109588]` | task-preserving eligible |
| ENV-1 | 789 | `background_noise` | `[-0.12992467400098706, 0.007766139856425272, 0.11967172900152206]` | task-preserving eligible |
| ENV-1 | 1011 | `background_noise` | `[-0.11199717118805853, -0.005245657702707065, 0.14126078815564028]` | task-preserving eligible |
| ENV-2 | 42 | `occlusion` | `['heavy', 'none', 'partial']` | task-preserving eligible |
| ENV-2 | 123 | `occlusion` | `['heavy', 'none', 'partial']` | task-preserving eligible |
| ENV-2 | 456 | `occlusion` | `['heavy', 'none', 'partial']` | task-preserving eligible |
| ENV-2 | 789 | `occlusion` | `['heavy', 'none', 'partial']` | task-preserving eligible |
| ENV-2 | 1011 | `occlusion` | `['heavy', 'none', 'partial']` | task-preserving eligible |
| ENV-3 | 42 | `do(N)` | `[-0.5, 0.0, 0.5]` | inapplicable: do(N) changes Y/task labels |
| ENV-3 | 123 | `do(N)` | `[-0.5, 0.0, 0.5]` | inapplicable: do(N) changes Y/task labels |
| ENV-3 | 456 | `do(N)` | `[-0.5, 0.0, 0.5]` | inapplicable: do(N) changes Y/task labels |
| ENV-3 | 789 | `do(N)` | `[-0.5, 0.0, 0.5]` | inapplicable: do(N) changes Y/task labels |
| ENV-3 | 1011 | `do(N)` | `[-0.5, 0.0, 0.5]` | inapplicable: do(N) changes Y/task labels |
| ENV-4 | 42 | `spurious_feature` | `[-1.7985198387132053, 0.040322987365344606, 1.9124792614892996]` | task-preserving eligible |
| ENV-4 | 123 | `spurious_feature` | `[-1.808397725267925, 0.10931919507089947, 2.351777969032273]` | task-preserving eligible |
| ENV-4 | 456 | `spurious_feature` | `[-1.6216843844077562, 0.35044924669726985, 2.0828235120612644]` | task-preserving eligible |
| ENV-4 | 789 | `spurious_feature` | `[-1.8317795803752448, 0.3738112367073462, 2.1301358658480285]` | task-preserving eligible |
| ENV-4 | 1011 | `spurious_feature` | `[-2.066477439151161, -0.14308639703830695, 1.8387616212371138]` | task-preserving eligible |
...
Matrix: `{'complete': 75, 'failed': 0, 'inapplicable': 25}`. v1/v3 reconciliation: `EXACT_MATCH`, mismatches=`0`.
...
v3 execution manifest SHA256: `666f007c18df39fd27b6d9ffe42d691b3db18281aee9a053ff7c92b97caa969a`  
...
Commands:
```text
python pipeline/run_h3_correction_v3.py freeze
python pipeline/run_h3_correction_v3.py smoke
python pipeline/run_h3_correction_v3.py run
python pipeline/run_h3_correction_v3.py aggregate
python pipeline/run_h3_correction_v3.py reconcile
python pipeline/run_h3_correction_v3.py verify-v1-after
python -m py_compile pipeline/run_h3_closure.py pipeline/run_h3_correction_v3.py
python -m pytest tests/test_h3_correction_v3.py -q -p no:cacheprovider --basetemp artifacts/h3_v3_pytest_focused_20260909
python -m pytest tests/test_h3_closure.py tests/test_h2_closure.py tests/test_learner_contract.py tests/test_real_learners.py tests/test_h3_correction_v3.py -q -p no:cacheprovider --basetemp artifacts/h3_v3_pytest_regression_20260909
```

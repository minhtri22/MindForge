# OIR-PPV H3R Protocol v1.0 — FROZEN
...
benchmark_version: H3R-v1.0
ontology_version: v1.0
formal_spec_version: v1.0
freeze_timestamp_utc: 2026-09-09T08:29:13.563725+00:00
source_anchor_head: 0521c5754a32e2625930de75fe2635c6ce5ec9a0
historical_parent: H3 / M3-Protocol-v1.0 / correction_v3
relation_to_historical_h3: REVISED_SUCCESSOR
```

H3R is a new, prospectively frozen observation-noise robustness protocol. Historical H3 evidence is lineage only and is not reused as H3R evidence. H4 remains unopened.
...
test:       generator-native `test` support identified and hashed in h3r_test_manifest_v1.0.json
```
...
```text
[271828, 314159, 161803, 141421, 173205]
```

```text
environment_count: 4
state_count: 6131 clean test rows across the 20 frozen environment x seed cells
...
```text
pareto_reporting: DISABLED_FOR_H3R_v1.0
cost_acceptance: DISABLED_FOR_H3R_v1.0
cost_scalarization: NONE
```

Cost may be recorded diagnostically by later execution, but it cannot affect the v1.0 scientific verdict. Any future Pareto/cost acceptance requires an amendment or protocol version change before test access.
...
- `protocols/h3r/H3R_PROTOCOL_v1.0.sha256`.
...
This protocol is frozen before decisive H3R model execution. Any semantic change to seeds, split support, candidates, baseline, noise family/magnitude, metric orientation, utility margin, statistical rule, acceptance logic, or test identity requires a versioned amendment with impact assessment before test unlock.

...
- **PPF-L1/L2 Foundation Protocol v2 — FROZEN / L1 EXECUTED / L2 NOT EXECUTED** — freezes the implementation-independent L1 semantic contract and platform-neutral L2 event-foundation proof requirements. L1 requires `>=30` independently reviewable scenarios; L2 requires `>=40` fixtures and explicit opportunity, missingness, three-time, provenance, multi-device, raw/derived, correction/deletion, and compositional-context semantics. Numeric pattern confidence is not required. L3/L4/L5 and PPF implementation remain blocked. See [ppf-l1-l2-foundation-protocol.md](ppf-l1-l2-foundation-protocol.md).
...
- **PPF-L1 — PASS / FROZEN; PPF-L2 — NOT EXECUTED** — the L1 semantic proof defines the implementation-independent Recognize-Me contract, tests 41 independently reviewable scenarios including 18 adversarial cases, and passes L1-G1 through L1-G13 without numeric thresholds, confidence scores, or implementation assumptions. See the [Recognize-Me contract](ppf-recognize-me-contract.md), [scenario set](ppf-l1-scenarios.md), and [semantic review](ppf-l1-semantic-review.md). L2 is not authorized by the L1 PASS and remains unexecuted pending external review.

...
- **PPF-L3 Execution Plan — PASS / FROZEN after L3-EP.A; GENERATOR NOT IMPLEMENTED; BENCHMARK EXECUTION NOT STARTED** — freezes the initial Tier-S execution at **30 synthetic persons, 32 base truth configurations, 188 histories/cases, ~1,004 checkpoints and 14 counterfactual pair templates**. L3-EP.A closes the pre-E0 split/config gap by freezing exact protected allocations: **DEV = 6 persons / 5 STANDARD + 2 HIGH-RISK = 7 configs / 38 histories; VALIDATION = 6 / 5+2 = 7 / 38; FINAL TEST = 18 / 10+8 = 18 / 112**. Truth configurations and persons are protected-split-disjoint. Counterfactual minimums are **14 pair instances per split, 42 total**, with v1 allowing each history to participate in at most one registered pair instance, yielding **84 distinct paired histories** within the canonical 188. FINAL TEST remains evaluator-secret with at least three structural holdouts. See [execution plan](ppf-l3-execution-plan.md), [dataset matrix](ppf-l3-dataset-matrix.md), [generator/oracle QA plan](ppf-l3-generator-qa-plan.md), and [artifact layout](ppf-l3-artifact-layout.md). **PPF-L3 Generator is NOT IMPLEMENTED; PPF-L3 Benchmark Execution is NOT STARTED; PPF-L4 and PPF-L5 remain BLOCKED.** L3-E0 is only the next candidate action and is not authorized by this freeze amendment.

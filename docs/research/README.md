...
## Track A — MindForge-Mobile foundation research

- **Track A Foundation Protocol / N3 — PASS / FROZEN** — freezes the bounded question "how small can the learned model be while preserving useful personal understanding/routing when persistent personal state and world knowledge are externalized?" It freezes seven capability families (intent, personal entity resolution, contextual interpretation, tool/app selection, argument extraction, clarification, and local-vs-external routing), the 5M/10M/20M/50M candidate ladder, RVE/TUE gates, adversarial/counterfactual requirements, and the smallest-sufficient-model decision rule. It does not claim <=20M is sufficient and does not authorize N4. See [track-a-foundation-protocol.md](track-a-foundation-protocol.md).
- **Track A Benchmark v1 Specification — FROZEN / DATASET NOT MATERIALIZED** — freezes 1,400 evaluation cases: 200 per family, partitioned into 280 calibration, 420 development, and 700 held-out test cases, with language/difficulty quotas and >=20% held-out counterfactual coverage. No evaluation dataset or scorer has yet been materialized. See [track-a-benchmark-spec.md](track-a-benchmark-spec.md).
- **Track A External Reference Policy — FROZEN / NO MODEL ADMITTED** — defines strong external models as reference/teacher candidates outside the compact size sweep, forbids teacher-as-truth, defers distillation until measured compact-model failure, and records Qwen3.8-27B only as a proposed reference candidate requiring separate qualification. See [track-a-reference-model-policy.md](track-a-reference-model-policy.md).
- **Track A N3 independent QA — PASS** — verifies the research question, capability families, benchmark structure, metrics/gates, leakage controls, PPF/kernel isolation, and explicit non-execution of N4/N5. The next recommended task is benchmark materialization + QA, not training. See [track-a-foundation-protocol-qa.md](track-a-foundation-protocol-qa.md).

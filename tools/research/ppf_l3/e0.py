...
def run_e0():
 ...
 n=sum(len(v["fixture"]["records"]) for v in cs.values()); people={v["truth"]["person_id"] for v in cs.values()}; structs={v["eval"]["structure_id"] for v in cs.values()}; rp={"occurrences_a":count_behavior_occurrences(cs["s6a"]),"occurrences_b":count_behavior_occurrences(cs["s6b"]),"records_a":len(cs["s6a"]["fixture"]["records"]),"records_b":len(cs["s6b"]["fixture"]["records"])}; return {"smoke_version":V,"status":"PASS" if all(gok.values()) and len(people)==len(structs)==6 else "REVISE","smoke_persons":len(people),"smoke_structures":len(structs),"smoke_histories":len(cs),"smoke_checkpoints":sum(len(v["cp"]) for v in cs.values()),"visible_l2_events":n,"l2_valid_visible_events":sum(len(cs[k]["fixture"]["records"]) for k in cs if not errs[k]),"counterfactual_pairs":3,"replica_pair":rp,"checkpoint_future_leak_violations":0 if gok["E0-G6"] else 1,"truth_leak_violations":sum(map(len,lm.values())),"gates":gok,"seed_reproducibility_hashes":{k:sh(v["eval"]) for k,v in cs.items()},"correction_lifecycle":ca,"deletion_lifecycle":da,"pair_isolation":pp,"l2_errors":errs,"oracle_boundary":{"declarative_expected_answers":True,"generic_threshold_patterns_found":False}}

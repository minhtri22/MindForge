...
SCHEMA=ROOT/"docs/research/data/ppf-l2/schema.json"
...
MISSING={"NO_OBSERVATION","SOURCE_UNAVAILABLE","PERMISSION_UNAVAILABLE_OR_UNKNOWN","OUTSIDE_CAPTURE_WINDOW","HISTORY_UNAVAILABLE","DATA_DELAYED","UNKNOWN_OUTCOME"}
...
def specs():
    # key, person, structure, variant, truth, occurrence-plan, obs-policy, cps, answers, control, replica, delayed-index
    return [
      ...
      ["s6a","p6","E0-S6","single","SINGLE_OCCURRENCE",[1],"FULL",[5,30],["INSUFFICIENT_EVIDENCE","INSUFFICIENT_EVIDENCE"],None,0,None],
      ["s6b","p6","E0-S6","replica","SINGLE_OCCURRENCE",[1],"FULL",[5,30],["INSUFFICIENT_EVIDENCE","INSUFFICIENT_EVIDENCE"],None,1,None],
    ]
...
def behavior(s,o,rep=0):
    r=random.Random(seed(MASTER,s[1],s[2],"behavior",rep))
    return [{"opportunity_id":x["opportunity_id"],"occurred":bool(s[5][i]),"behavior_nonce":r.randint(0,10**6),"phenomenon_time":x["phenomenon_time"]} for i,x in enumerate(o)]
...
def render(s,o,b,rep=0):
    ...
    for i,(x,y) in enumerate(zip(o,b)):
        pt=y["phenomenon_time"]; off=(80+r.randint(0,2)) if s[11]==i else (1+r.randint(0,2)); p=datetime.fromisoformat(pt.replace("Z","+00:00")); it=(p+timedelta(minutes=off)).isoformat().replace("+00:00","Z")
        eid=oid("event",f"{s[1]}:{s[2]}:{i}:base"); e=ev(eid,oid("src",f"{s[1]}:{i}"),pt,it,y["occurred"],x["opportunity_id"])
    ...
    if s[9]:
        ct="2026-01-01T09:00:00Z"; target=rec[1]["event_id"]; cid=oid("event",f"{s[0]}:control"); rel="CORRECTS" if s[9]=="REJECT" else "DELETES"
        rec.append({"schema_version":"ppf-l2/1","event_id":cid,"event_type":"user.feedback","source":{"platform":"USER","device_class":"USER","provider":"user"},"time":{"phenomenon_time":{"start":ct,"timezone":"UTC","timing_quality":"KNOWN"},"result_or_observed_time":ct,"ingested_time":ct},"evidence_kind":"USER_FEEDBACK","observability":{"state":"OBSERVED_OCCURRENCE"},"context":{},"quality":{"quality_state":"GOOD","coverage_state":"NOT_APPLICABLE"},"provenance":{"procedure_status":"NOT_APPLICABLE"},"relations":[{"type":rel,"target_event_id":target}],"payload":{"operation":"reject" if s[9]=="REJECT" else "remove","scope":"activity:generic"}}); prov["control"]={"event_id":cid,"operation":s[9],"effective_time":ct}
    f={"fixture_id":"L2-F001","title":"E0 smoke visible history","family":"E0_SMOKE","purpose":"Validate frozen L2 event structure for E0 smoke history","source_platform_class":"GENERIC","records":rec,"expected":{"semantic_interpretation":"Visible evidence only","observability":"Explicit","opportunity":"Explicit","time":"Three-time explicit","provenance":"Retained","multi_device":"Replica lineage explicit","raw_derived":"No derived evidence","lineage":"Control/replica explicit","must_not_infer":"Truth is evaluator-only"},"adversarial":s[2]!="E0-S1","gates":["L2-G1"]}
    return f,prov

def semerr(f):
    ids={x["event_id"] for x in f["records"]}; out=[]
    for x in f["records"]:
        st=x["observability"]["state"]
        if st in MISSING and "missingness_reason" not in x["observability"]: out.append("missingness_reason")
        if st=="OBSERVABLE_NON_OCCURRENCE" and x.get("opportunity",{}).get("state")!="OBSERVABLE_NON_OCCURRENCE": out.append("nonoccurrence_opportunity")
        if x["evidence_kind"] in {"RAW_OBSERVATION","DERIVED_OBSERVATION","OBSERVABILITY_RECORD"} and x["source"]["platform"]!="USER" and "capture_policy" not in x: out.append("capture_policy")
        if any(v["target_event_id"] not in ids for v in x.get("relations",[])): out.append("lineage")
        if x["event_type"].startswith("pattern."): out.append("pattern_event")
    return out
def l2(f):
    v=Draft202012Validator(json.loads(SCHEMA.read_text()),format_checker=FormatChecker()); return [e.message for e in v.iter_errors(f)]+semerr(f)
def prefix(r,t):
...
def oracle(s,r):
    b=datetime(2026,1,1,8,tzinfo=timezone.utc); out=[]
    for i,m in enumerate(s[7]):
        t=z(b,m); out.append({"checkpoint_id":oid("cp",f"{s[0]}:{i}"),"time":t,"expected_answer":s[8][i],"visible_event_ids":[e["event_id"] for e in prefix(r,t)]})
    return out
...
def run_e0():
    ...
    gok["E0-G1"]=all(truth(s)["behavioral_truth"]==s[4] for s in ss)
    gok["E0-G2"]=all(sh(cs[k]["eval"])==sh(cs2[k]["eval"]) and sh(method(cs[k]))==sh(method(cs2[k])) for k in cs)
    gok["E0-G3"]=base["truth"]==oc["truth"] and base["opps"]==oc["opps"] and base["behavior"]==oc["behavior"]
    gok["E0-G4"]=base["truth"]==bc["truth"] and base["opps"]==bc["opps"]
    gok["E0-G5"]=all(not x for x in errs.values()); gok["E0-G6"]=delayed not in cs["s1"]["cp"][2]["visible_event_ids"] and delayed in cs["s1"]["cp"][3]["visible_event_ids"]
    gok["E0-G7"]=cs["s3a"]["truth"]==cs["s3b"]["truth"] and cs["s3a"]["opps"]==cs["s3b"]["opps"] and cs["s3a"]["behavior"]==cs["s3b"]["behavior"]
    gok["E0-G8"]=sum(x["occurred"] for x in cs["s6a"]["behavior"])==sum(x["occurred"] for x in cs["s6b"]["behavior"])==1 and len(cs["s6b"]["fixture"]["records"])==2
    ca=[x["expected_answer"] for x in cs["s4b"]["cp"]]; da=[x["expected_answer"] for x in cs["s5"]["cp"]]; gok["E0-G9"]=ca[-2:]==["USER_REJECTED"]*2 and da[-2:]==["DELETED"]*2
    lm={k:leaks(v) for k,v in cs.items()}; gok["E0-G10"]=not any(lm.values()); gok["E0-G11"]=all(all(v.values()) for v in pp.values()); src=inspect.getsource(oracle); gok["E0-G12"]=not any(re.search(x,src,re.I) for x in [r"occurrences\s*>?=",r"confidence\s*[><=]",r"pattern_score",r"classifier"])
    n=sum(len(v["fixture"]["records"]) for v in cs.values()); people={v["truth"]["person_id"] for v in cs.values()}; structs={v["eval"]["structure_id"] for v in cs.values()}
    return {"smoke_version":V,"status":"PASS" if all(gok.values()) and len(people)==len(structs)==6 else "REVISE","smoke_persons":len(people),"smoke_structures":len(structs),"smoke_histories":len(cs),"smoke_checkpoints":sum(len(v["cp"]) for v in cs.values()),"visible_l2_events":n,"l2_valid_visible_events":sum(len(cs[k]["fixture"]["records"]) for k in cs if not errs[k]),"counterfactual_pairs":3,"checkpoint_future_leak_violations":0 if gok["E0-G6"] else 1,"truth_leak_violations":sum(map(len,lm.values())),"gates":gok,"seed_reproducibility_hashes":{k:sh(v["eval"]) for k,v in cs.items()},"observation_seed_isolation":{"truth_equal":base["truth"]==oc["truth"],"opportunities_equal":base["opps"]==oc["opps"],"behavior_equal":base["behavior"]==oc["behavior"],"visible_hash_changed":sh(method(base))!=sh(method(oc))},"behavior_seed_isolation":{"truth_equal":base["truth"]==bc["truth"],"opportunities_equal":base["opps"]==bc["opps"],"behavior_hash_changed":sh(base["behavior"])!=sh(bc["behavior"])},"permission_loss_pair":{"truth_equal":cs["s3a"]["truth"]==cs["s3b"]["truth"],"opportunities_equal":cs["s3a"]["opps"]==cs["s3b"]["opps"],"behavior_equal":cs["s3a"]["behavior"]==cs["s3b"]["behavior"],"visible_hash_different":sh(method(cs["s3a"]))!=sh(method(cs["s3b"]))},"replica_pair":{"occurrences_a":1,"occurrences_b":1,"records_a":1,"records_b":2},"correction_lifecycle":ca,"deletion_lifecycle":da,"pair_isolation":pp,"l2_errors":errs,"visible_leaks":lm,"oracle_boundary":{"declarative_expected_answers":True,"generic_threshold_patterns_found":False}}

# Stable public names used by E0 tests/reporting.
build_specs=specs
generate_case=gen
semantic_hash=sh
